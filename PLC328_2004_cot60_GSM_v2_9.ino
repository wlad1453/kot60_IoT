...
boolean dailySMSsend(false);
String dailySMS(""), alarmSMS("");
uint8_t smsHour(18), smsMin(0);
...
void setup()   /*----( SETUP: RUNS ONCE )----*/
  ...
  while( gsm.status()!= GSM_OK ) { Serial.println(" OK!"); delay(1000); };                           // Ждём завершения регистрации модема в сети оператора связи.
  Serial.println(" OK!");
 
  // Serial.println( gsm.runUSSD("*100#") );     // Вывести баланс (ответ на команду *100# ).  
  // gsm.SMSsend(F("1 2 3"), "79219626775"); 
...
void loop()   /*----( LOOP: RUNS CONSTANTLY )----*/
{
  ...
  if (day && (T[0] < 20.0) && !dayHeaterOn && !relayOn) { // If there is day time and the upper temp falls under 15°C
    dayHeating(1);                                        // Switch on the day heater (in the upper part of the heat accumulator)
    dayHeaterOn = true;
  } 
  else if (dayHeaterOn && (!day || (T[0] > 30.0)) ) {     // While at night OR the UTemp has risen over 30°C
    dayHeating(0);                                        // The day heater is switched off
    dayHeaterOn = false;
  }
  ...
  // gsm.SMSsend( alarmSMS, "79219626775");

  if (!dailySMSsend && (hours == 18)) {             
    gsm.SMSsend( dailySMS, "79219626775");
    dailySMSsend = true;
    dailySMS = "";
    }
  if (dailySMSsend && (hours == 0)) dailySMSsend = false;
...
void nightHeating (boolean on) {     // Relay switch ON (LOW) / OFF (HIGH)
  ...
  if (on) {
    ...
    buzzerPeep (BUZZER, 100);
    PORTD |= (1 << relay1);              // digitalWrite (relay1, HIGH);
    buzzerPeep (BUZZER, 100);
    PORTD |= (1 << relay2);              // digitalWrite (relay2, HIGH);
    buzzerPeep (BUZZER, 100);
